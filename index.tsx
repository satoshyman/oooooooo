...
import { AppState, View, Task, Referral, AppConfig, WithdrawalRequest } from './types';
import { INITIAL_TASKS, NAV_ITEMS, TON_LOGO } from './constants';
...
const LOCAL_STORAGE_KEY = 'ton_miner_premium_v2';
...
const App: React.FC = () => {
  ...
  // Fix: Safe access to Telegram HapticFeedback using optional chaining
  ...
  const [state, setState] = useState<AppState>(() => {
    try {
      ...
      if (saved) {
          ...
          // تأكد من اندماج البيانات القديمة مع الإعدادات الجديدة لتجنب الانهيار
          return {
              ...parsed,
  ...
  // Fix: Initialize Telegram WebApp properties with safety checks
  useEffect(() => {
  ...
  // Fix: Use HapticFeedback with optional chaining for secret toggle feedback
    ...
    const timer = setInterval(() => {
      setState(prev => {
        ...
        if (newSessionStartTime && (now - newSessionStartTime >= prev.config.sessionDuration)) {
          ...
          showToast(`Mining Finished: +${prev.config.miningRate} TON`);
        }
  ...
  // Fix: Safe access to HapticFeedback for view changes
  ...
  return (
    <div className="flex flex-col h-screen max-w-md mx-auto relative text-white overflow-hidden select-none">
      {toast && (
      ...
      <main className="flex-1 px-5 pb-32 pt-4 overflow-y-auto scroll-smooth">
        {currentView === View.MINING && (
          <MiningView 
            ...
            onClaimDaily={() => { setState(p => ({ ...p, stats: { ...p.stats, balance: p.stats.balance + p.config.dailyGiftReward, lastDailyGiftClaimed: Date.now() } })); showToast('Reward Claimed!'); }} 
            onClaimFaucet={() => { setState(p => ({ ...p, stats: { ...p.stats, balance: p.stats.balance + p.config.faucetReward, lastFaucetClaimed: Date.now() } })); showToast('Faucet Filled!'); }}
            onAdBonus={() => { setState(p => ({ ...p, stats: { ...p.stats, balance: p.stats.balance + p.monetization.adBonus } })); }}
            onWithdrawal={(wallet, amount) => { setState(prev => ({ ...prev, stats: { ...prev.stats, balance: prev.stats.balance - amount }, withdrawals: [...prev.withdrawals, { id: Date.now().toString(), username: 'User', walletAddress: wallet, amount, status: 'Pending', requestedAt: new Date().toLocaleString() }] })); showToast('Request Sent'); }}
          />
        )}
        {currentView === View.REFERRALS && <ReferralView referrals={state.referrals} code={state.referralCode} />}
        {currentView === View.TASKS && <TasksView tasks={state.tasks} onComplete={(id) => { setState(prev => ({ ...prev, stats: { ...prev.stats, balance: prev.stats.balance + (prev.tasks.find(t => t.id === id)?.reward || 0) }, tasks: prev.tasks.map(t => t.id === id ? { ...t, completed: true } : t) })); showToast('Task Complete'); }} />}
      ...
      {isPasscodePromptOpen && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-xl z-[3000] flex items-center justify-center p-6 animate-in zoom-in-95">
           <div className="w-full glass-card rounded-[2.5rem] p-8 space-y-6 relative border-white/10">
              ...
              <div className="flex flex-col items-center gap-4 text-center">
                ...
                <h2 className="text-xl font-black uppercase">Admin Access</h2>
              </div>
              <input 
              ...
              />
              <button onClick={verifyPasscode} className="w-full bg-red-600 hover:bg-red-500 text-white font-black py-4 rounded-2xl text-sm uppercase transition-transform active:scale-95">Verify</button>
           </div>
        </div>
      )}
    </div>
